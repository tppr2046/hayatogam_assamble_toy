...
--   鋪一層 Bayer 白點把零件打散成灰，標題才讀得到。
--
-- ★★ `setDitherPattern` 的 alpha 是**透明度**，不是不透明度 —— **數字越大、白點越少**。
--   2026-08-13 用開機 log 實測（16x16 取樣 256 點）：
--     0.00 → 256/256（整片全白）  0.25 → 192  0.50 → 128  0.75 → 64  1.00 → 0（完全不畫）
--   ⚠️ 舊註解寫「設 0 = 不減淡」是**錯的**：0 之所以看起來沒事，只是因為下面的
--     `if BG_DIM > 0` 把整段跳過了。真的寫 0.01 會得到幾乎全白的畫面。
--   → **要讓零件更清楚就調大；要標題更好讀就調小。**
local BG_DIM        = 0.65   -- 白點 128 → 90（少約 30%），零件稍微清楚一些
