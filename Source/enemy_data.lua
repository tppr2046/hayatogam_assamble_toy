...
return {
    ...
    ["RAMMER_ENEMY"] = {
        ...
        -- [[ 2026-09-22 ]] 專屬圖 **enemy05-table-40-32.png（3 格）**，不再與 SHIELD_ROBOT 共用 enemy02。
        --   1＝待機**與**移動（同一格）　2~3＝推擊時**播一次**（推板沿伸縮桿往前推出）
        -- ★ 推板在圖的**左側**＝面向左，與本作素材的基準朝向一致 → 不需要 flip_x。
        --   玩家在右邊時整張會被鏡射，推板就會朝右推向玩家。
        -- ★ 圖寬由 32 變 40 → 命中框也跟著變寬（命中框是讀圖算的）。
        -- ★ 移動速度完全沒動：衝向玩家時 chase_speed 58、走回原點時 return_speed 30 ——
        --   玩家感覺到的「推動時加速」就是這個差距。撞到的瞬間本身**沒有**額外加速。
        image = "images/enemy05",
        anim_idle_move = true,
        idle_frame  = 1,
        walk_frames = { 1 },       -- 移動時也用第 1 格
        push_frames = { 2, 3 },    -- 撞到機體時播一次
        push_fps    = 8,           -- 每格 0.125 秒 → 整段約 0.25 秒
        bullet_offset_x = 4,
        bullet_offset_y = 16
    }
}