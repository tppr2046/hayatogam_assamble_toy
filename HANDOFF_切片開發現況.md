...
### ★ 3-3d. 動過圖檔名之後,跑 `tools/check_enemy_images.py`(2026-08-20 新增)

```powershell
python tools/check_enemy_images.py
```
印出**敵人 / BOSS ↔ 圖檔**的完整對照(也就是 [ArtAssets §0.7](ArtAssets_切片_組裝玩具任務.md) 那張表),
並檢查兩件事:**缺圖**(引用了不存在的檔)與**孤兒圖**(畫了但沒人讀)。有缺圖時離開碼回 1。

⚠️ **為什麼需要它**:某隻敵人指到不存在的圖時,`pdc` **不會擋、開機也不會報錯** ——
它會安靜地退回黑方塊佔位,只有實際進到那一關才看得到。
...
### ★ 6-0b. 鬆動平台的「重生秒數」(2026-09-26 新增)

平台屬性多了 **重生秒數**:填了就會在塌掉那麼久之後重新產生(等待期間畫虛線輪廓),
留空/0＝**塌了就永久消失**。兩種都可以用。
⚠️ `sceneToJSON` 是白名單輸出,**新增欄位一定要同時補那一行**,否則會在「載入→重存」時靜默消失。
...
沒連結時退回 `BG_FALLBACK`(寫死的清單 + 尺寸),所以換台電腦開也不會壞。
> ⚠️ **`BG_FALLBACK` 會過期**:連結資料夾的人自動看得到新圖,**沒連結的人只看得到這份**。
> 加背景圖時要順手補它(2026-09-24 補了 `bg_building8`/`9`,各 200×70,現在共 9 張)。
