...
1. **M001–M003 的資源數值**還沒調(獎勵曲線要整體設計,不是逐關拍腦袋)
2. **§4-2b 的兩個測試用暫時物**還在:`"final": true` @ M008、`CORE(test)` 系統選單項
3. **分支 `feat/g2-hq-layout` 領先 origin 18 筆、從未 push** —— 要不要 push 由使用者決定
